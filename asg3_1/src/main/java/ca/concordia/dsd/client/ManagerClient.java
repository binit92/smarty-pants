...
public class ManagerClient implements Constants {
        ...
        if (managerId != null && managerId.length() == 7) {
            if (Pattern.compile("[0-9]*").matcher(managerId.substring(3, 6)).matches()) {
                ...
                if (firstThree.equalsIgnoreCase(MTL_TAG)) {
                    server =  createConnection(MTL_TAG, MTL_SERVER_HOST, MTL_SERVER_PORT);
                    return true;
                } else if (firstThree.equalsIgnoreCase(LVL_TAG)) {
                    server  = createConnection(LVL_TAG, LVL_SERVER_HOST, LVL_SERVER_PORT);
                    return true;
                } else if (firstThree.equalsIgnoreCase(DDO_TAG)) {
                    server =  createConnection(DDO_TAG, DDO_SERVER_HOST, DDO_SERVER_PORT);
                    return true;
                }
    ...
    // Putting commands arguments programmatically here:
    // java HelloClient -ORBInitialPort 1050 -ORBInitialHost localhost
    private CenterServerImpl createConnection(String tag, String name, int port) {
        try {
            logUtil.log("createconnection  tag: " + tag + " name: " + name + " port: " + port);
            switch(tag) {
            	case Constants.MTL_TAG:
            		MTLCenterServerServiceLocator MTLlocator = new MTLCenterServerServiceLocator();
            		return (CenterServerImpl) MTLlocator;
            	case Constants.LVL_TAG:
            		LVLCenterServerServiceLocator LVLlocator = new LVLCenterServerServiceLocator();
            		return (CenterServerImpl) LVLlocator;
            	case Constants.DDO_TAG:
            		DDOCenterServerServiceLocator DDOlocator = new DDOCenterServerServiceLocator();
            		return (CenterServerImpl) DDOlocator;
            }
        ...
        while (i != 0) {
            ...
            switch (input) {
                ...
                case 5:
					...
					try {
						isSuccess = server.transferRecord(managerId,rID,rServerName);
					} catch (RemoteException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
                    //String isSuccess = "asd";
