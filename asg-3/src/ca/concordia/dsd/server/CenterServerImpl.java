...
public class CenterServerImpl implements ICenterServer {
                        ...
                        if(found.getUniqueId().compareTo(recordId) == 0){
                            ...
                            try{
                                ...
                                if(found.getType() == Records.RecordType.TEACHER){
                                    ...
                                    List<String> courses = sr.getCoursesRegistered();
                                    for(String course : courses) {
                                    	br.append(courses);br.append(",");
                                    }
                                    br.append("|");
    ...
    private synchronized String editSRRecord(String manager,String recordID, String key, String val) {
	    ...
	    Records rec = getRecordUsingId(recordID);
	    if (rec != null) {
	    	synchronized(rec) {
	    		if ( key.equals("Status")) {
                    ((StudentRecord) rec).setStatus(val);
                    logUtil.log("Records update for : " + serverName);
                    return "Records updated with status : "+val;
                } else if (key.equals("StatusDate")) {
                    ((StudentRecord) rec).setStatusDate(val);
                    logUtil.log("Records update for : " + serverName);
                    return "Records updated with status : "+val;
                }else if(key.equalsIgnoreCase("courses")) {
                	List<String> courses = new ArrayList();
                	String arr[] = val.split(",");
                	for(String entry: arr) {
                		courses.add(entry);
                	}
                    ((StudentRecord) rec).setCoursesRegistered(courses);
                    logUtil.log("Records update for : " + serverName);
                }
	    	}
	    }
 	    ...
 	    if (rec != null) {
 	    	synchronized(rec) {
 	    		if (key.equalsIgnoreCase("Phone")) {
                    ((TeacherRecord) rec).setPhone(val);
                    logUtil.log(manager,"Records update for : " + serverName);
                     return "Records updated with status : "+val;
                 }

                 else if ( key.equalsIgnoreCase("Address")) {
                     ((TeacherRecord) rec).setAddress(val);
                     logUtil.log(manager,"Records update for : " + serverName);
                     return "Records updated with status : "+val;
                 }

                 else if (key.equalsIgnoreCase("Location")) {
                     ((TeacherRecord) rec).setLocation(val);
                     logUtil.log(manager,"Records update for : " + serverName);
                     return "Records updated with status : "+val;
                 }
 	    	}
    ...
    private synchronized Records getRecordUsingId(String recID) {
		for (List<Records> recordList :  recordsMap.values())
		{	
			for (Records record : recordList) 
			{
				if(record.getUniqueId().equalsIgnoreCase(recID)){
					return record;
				}
			}
		}
